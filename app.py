# FEHB + FEDVIP 2026 Custom Estimator (v2.1c)
# Streamlit app with full inputs and robust FEDVIP numeric parsing
...
st.markdown("<h1 style='text-align:center;'>💼 FEHB + FEDVIP 2026 – Custom Estimator (v2.1c)</h1>", unsafe_allow_html=True)
...
def _to_float(x, default=0.0):
    ...
    try:
        if s.lower().startswith('no'):  # "No charge", etc.
            return 0.0
...
with st.sidebar:
    ...
    dent_prem = st.number_input("Dental monthly premium ($)", min_value=0.0,
                                value=dent_default if include_dental else 0.0, step=1.0, format="%.0f")
    vis_prem = st.number_input("Vision monthly premium ($)", min_value=0.0,
                                value=vis_default if include_vision else 0.0, step=1.0, format="%.0f")
...
for _, r in df.iterrows():
    ...
    if include_dental:
        ...
        if major_dental or crowns > 0:
            ...
            plan_pays = min(2000.0, 0.5 * crowns_cost)  # assume $2k annual max and 50% major
...
st.caption("© 2025 Mason Magle | FEHB + FEDVIP 2026 Custom Estimator v2.1c | Data: OPM Public Use Files | Heuristic OOP model; verify with SBC.")
